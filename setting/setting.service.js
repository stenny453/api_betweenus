...
let SettingService = class SettingService {
    constructor(settingRepository, modelService, clientService) {
    ...
    async getSettingClient(client) {
        const currentClient = await this.clientService.getClient(client.id);
        if (!currentClient)
            return false;
        const result = __rest(currentClient.setting, []);
        if (!result.sound_message)
            return await this.createSettingClient(client.id);
        return result;
    }
    ...
    async createSettingClient(clientId) {
